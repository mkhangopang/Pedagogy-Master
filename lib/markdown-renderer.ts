...
export function markdownToHtml(md: string): string {
  ...
  html = html.replace(
    /```[a-z]*\n?([\s\S]*?)```/g,
    (_m, code) =>
      '<pre style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:12px 16px;margin:12px 0;overflow-x:auto;font-size:12px;font-family:monospace;white-space:pre-wrap">' +
      code +
      '</pre>'
  );
