...
import {
  ...
  Terminal, AlertTriangle, Zap, Loader2,
...
export const DocumentReader: React.FC<DocumentReaderProps> = ({ document: activeDoc, onClose }) => {
  ...
  // ── Grade-first hierarchy ───────────────────────────────────────────────────
  // Grade 09 → [Domain A, B, C...] → Grade 10 → ...
  ...
  const gradeHierarchy = useMemo(() => {
    ...
    // Sort grades numerically (09, 10, 11, 12), Ungraded last
    ...
    return (
      <div className="fixed inset-0 z-[500] bg-[#020202] flex items-center justify-center p-6 animate-in fade-in duration-300">
        <div className="w-full max-w-lg bg-[#0d0d0d] rounded-[2.5rem] border border-white/5 p-10 shadow-2xl">
          ...
          {processError && (
            <div className="p-4 bg-rose-950/30 border border-rose-800/30 rounded-2xl text-rose-400 text-xs mb-6">
              {processError}
            </div>
          )}
          <div className="space-y-2">
            {STEPS.slice(0, 4).map((step, i) => {
              ...
              return (
                <div
                  key={step.key}
                  className={`flex items-center gap-3 text-[9px] font-black uppercase tracking-widest ${done ? 'text-emerald-500' : active ? 'text-indigo-400' : 'text-white/15'}`}
                >
          ...
          {processDone && (
            <button
              ...
              className="mt-8 w-full py-4 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-700 transition-all flex items-center justify-center gap-2"
            >
        ...
        <div className="max-w-6xl mx-auto p-6 md:p-10">
          ...
          {loading ? (
            ...
            <div className="space-y-6 animate-in slide-in-from-bottom-4">
              ...
              {/* Grade / domain breakdown */}
              <div className="bg-white dark:bg-[#080808] rounded-2xl border border-slate-100 dark:border-white/5 p-8">
                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-900 dark:text-white mb-6">Grade → Domain Breakdown</h3>
                        ...
                        <div className="space-y-3">
                          {items.map(slo => {
                            ...
                            return (
                              <div
                                key={slo.id}
                                className={`group bg-white dark:bg-[#080808] rounded-2xl border transition-all duration-200 hover:shadow-lg ${borderClass}`}
                              >
            ...
            <div className="flex flex-col items-center justify-center py-40 text-center animate-in zoom-in-95 duration-500">
              ...
              <div className="flex gap-3 mt-10">
                <button
                  onClick={fetchSlos}
                  className="flex items-center gap-2 px-6 py-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all"
                >
                  <RefreshCw size={14} /> Refresh
                </button>
                <button
                  onClick={runProcessing}
                  className="flex items-center gap-2 px-8 py-4 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl hover:bg-indigo-700 active:scale-95 transition-all"
                >
      ...
      <footer className="h-10 border-t dark:border-white/5 bg-white dark:bg-[#080808] flex items-center justify-between px-8 shrink-0">
        ...
        <div className="flex items-center gap-4">
          {slos.length > 0 && (
            <button
              onClick={runProcessing}
              className="text-[8px] font-black text-slate-400 hover:text-indigo-500 uppercase tracking-widest transition-all flex items-center gap-1"
            >
