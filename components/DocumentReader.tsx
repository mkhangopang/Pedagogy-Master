...
export const DocumentReader: React.FC<DocumentReaderProps> = ({ document, onClose }) => {
  ...
  const renderedHtml = useMemo(() => {
    ...
    // Detects lines starting with bullets or just SLO text and wraps them in a stylized div structure
    // This regex looks for the pattern and replaces it with a block that can be rendered inside the list item or paragraph
    const sloRegex = /(?:^|\n)(?:[-•*]\s*)?((?:SLO|LO)[:\s]+[A-Z0-9\.-]{3,18})[:\s-]*([^\n]+)/gi;
    
    text = text.replace(sloRegex, (match, code, desc) => {
        const cleanCode = code.replace(/^(?:SLO|LO)[:\s]*/i, '').trim();
        // We output a div structure that will be valid HTML. 
        // Note: If this sits inside a <p> tag from marked, it might be split, but usually marked handles block level replacements if we use double newlines.
        // For inline replacement, we use spans with block display classes.
        ...
        return `\n\n<div class="slo-card-container my-4 group relative pl-4 border-l-4 border-indigo-500 bg-slate-50 dark:bg-white/5 rounded-r-xl p-4 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all cursor-pointer slo-interactive-pill" data-slo="${cleanCode}">
          <div class="flex flex-col sm:flex-row sm:items-center gap-3">
             <span class="inline-flex items-center px-3 py-1.5 rounded-lg bg-indigo-600 text-white font-black text-[10px] tracking-widest shadow-md shrink-0 whitespace-nowrap">
               SLO: ${cleanCode}
             </span>
          ...
          <div class="absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity">
             <span class="p-1.5 bg-white dark:bg-black rounded-lg text-indigo-500 shadow-sm text-[9px] font-bold flex items-center gap-1">
               <span class="w-3 h-3"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg></span> Copy
             </span>
          </div>
        </div>\n\n`;
    });
  ...
  const handleContainerClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (pill) {
      ...
      if (sloCode) {
        // Attempt to grab full text of the card
        const fullText = pill.textContent?.trim().replace(/\s+/g, ' ') || sloCode;
        navigator.clipboard.writeText(fullText); // User requested to copy the SLO text too
  ...
  const triggerFind = () => {
    // Simple browser find trigger or visual cue
    setShowFind(!showFind);
    // In a real app, we might implement a custom search highlight, 
    // but for now we'll rely on native 'Ctrl+F' behavior suggestion or focusing.
    // Let's just focus the reader
  ...
  return (
    <div className="fixed inset-0 z-[500] bg-slate-50 dark:bg-[#050505] flex flex-col animate-in fade-in zoom-in-95 duration-300">
      ...
      <div className="fixed bottom-6 left-4 right-4 md:left-1/2 md:-translate-x-1/2 md:w-auto z-[550]">
        <div className="flex items-center justify-between md:justify-center gap-4 bg-slate-900/95 dark:bg-[#1a1a2e]/95 backdrop-blur-xl px-6 py-4 md:px-10 md:py-5 rounded-2xl md:rounded-[2.5rem] shadow-[0_20px_60px_rgba(0,0,0,0.5)] border border-white/10 animate-in slide-in-from-bottom-10 duration-700">
