...
export default function DocumentUploader({ userId, onComplete, onCancel }: DocumentUploaderProps) {
  ...
  const synthesizeMasterMarkdown = async (rawText: string, fileName: string) => {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: [{ role: 'user', parts: [{ text: `Synthesize Master Markdown for: ${fileName}\n\nINPUT:\n${rawText.substring(0, 35000)}` }] }],
      config: {
        temperature: 0.1,
        systemInstruction: "You are an institutional data architect. Structure curriculum using '#' for Units, '## Learning Outcomes', and '### Standard: [ID]' for RAG indexing."
      }
    });
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl lg:rounded-[3rem] p-1 w-full max-w-6xl shadow-2xl border border-slate-100 dark:border-white/5 animate-in zoom-in-95 flex flex-col h-[90vh] lg:h-[85vh] overflow-hidden">
        <div className="flex items-center justify-between p-4 lg:p-8 border-b dark:border-white/5">
          <div className="flex items-center gap-4">
            <button onClick={goBackToSelection} className="p-2 lg:p-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all group" title="Back to Selection">
        ...
        <div className="flex-1 flex flex-col lg:grid lg:grid-cols-2 overflow-hidden">
          <div className="flex flex-col border-b lg:border-b-0 lg:border-r dark:border-white/5 p-4 lg:p-8 bg-slate-50/50 dark:bg-black/20 h-1/2 lg:h-full">
            <label className="text-[10px] font-black uppercase tracking-widest text-indigo-500 mb-3 flex items-center gap-2">
              <ShieldCheck size={12}/> Editable Source
            </label>
            <textarea value={draftMarkdown} onChange={(e) => {setDraftMarkdown(e.target.value); setError(null);}} className="flex-1 p-4 lg:p-6 bg-white dark:bg-slate-950 border border-slate-200 dark:border-white/10 rounded-2xl lg:rounded-3xl font-mono text-[11px] lg:text-xs leading-loose outline-none focus:ring-2 focus:ring-indigo-500 shadow-inner resize-none" readOnly={isProcessing} />
          </div>
          <div className="flex flex-col p-4 lg:p-8 bg-white dark:bg-slate-900 overflow-y-auto custom-scrollbar h-1/2 lg:h-full">
            ...
            <div className="prose dark:prose-invert max-w-none text-xs lg:text-sm" dangerouslySetInnerHTML={{ __html: previewHtml || '<p>Generating...</p>' }} />
          </div>
        </div>
        ...
        <div className="p-4 lg:p-8 border-t dark:border-white/5 bg-slate-50 dark:bg-slate-900/50 flex flex-col lg:flex-row items-center justify-between gap-4">
          <div className="w-full lg:max-w-md">
            ...
            {!error && <p className="text-[10px] lg:text-xs text-emerald-600 font-bold flex items-center gap-2"><CheckCircle2 size={14}/> {draftMarkdown ? 'Adaptive Structure Synced.' : 'Synthesis Pending.'}</p>}
          </div>
          <div className="flex w-full lg:w-auto gap-3">
            <button onClick={goBackToSelection} disabled={isProcessing} className="flex-1 lg:flex-none px-6 py-3 text-slate-400 font-bold hover:text-slate-700 dark:hover:text-slate-200 transition-colors text-sm disabled:opacity-50">Back</button>
            <button onClick={handleFinalApproval} disabled={isProcessing || !draftMarkdown} className="flex-1 lg:flex-none px-8 lg:px-12 py-3 bg-indigo-600 text-white rounded-xl lg:rounded-2xl font-black shadow-xl shadow-indigo-600/30 hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50 text-sm">
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-[2rem] lg:rounded-[3rem] p-6 lg:p-12 w-full max-w-2xl shadow-2xl border border-slate-100 dark:border-white/5 animate-in zoom-in-95 relative overflow-hidden">
      ...
      <button onClick={onCancel} className="mt-8 lg:mt-10 w-full py-4 text-slate-400 font-bold hover:text-slate-600 transition-colors uppercase tracking-widest text-[10px]">Disconnect Node</button>
      ...
      {isProcessing && (
        <div className="absolute inset-0 bg-white/95 dark:bg-slate-950/95 flex flex-col items-center justify-center rounded-3xl lg:rounded-[3rem] z-50 backdrop-blur-md">
          <Loader2 className="animate-spin text-indigo-600 mb-4 w-10 h-10" />
          <p className="text-base font-black tracking-tight text-indigo-600">Adaptive Ingestion Active...</p>
        </div>
      )}
    </div>
  );
