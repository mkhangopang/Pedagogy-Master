...
export default function DocumentUploader({ userId, onComplete, onCancel }: DocumentUploaderProps) {
  ...
  const extractRawText = async (file: File, type: 'pdf' | 'docx'): Promise<string> => {
    try {
      ...
      if (type === 'pdf') {
        ...
        // Iterative extraction to prevent memory overhead on large files
        ...
        console.log(`✅ [Extractor] Extracted ${fullText.length} characters.`);
        return fullText;
      } else {
  ...
  const synthesizeMasterMarkdown = async (rawText: string, fileName: string) => {
    ...
    if (!apiKey) throw new Error("Neural synthesis node offline: API key missing.");

    try {
      const ai = new GoogleGenAI({ apiKey });
      
      // EXPANDED CAPACITY: Substring increased to 250,000 characters to capture full 100+ page docs
      ...
      CRITICAL CAPACITY RULE:
      I am providing a very large document (${processingText.length} characters). 
      You MUST process the ENTIRE text provided, including the final sections (Glossaries, Appendices, References).
      
      STRICT STRUCTURAL RULES:
      1. First Header: '# Curriculum Metadata'.
      2. Metadata must include: 'Board:', 'Subject:', 'Grade:', 'Version:'.
      3. Chapters/Units: Use '#' headers.
      4. Standards: Use '### Standard: [ID]' for RAG-indexable blocks.
      5. SLOs: Format verbatim as '- SLO: [CODE]: [Description]'.
      6. Remove noise like 'Page X of Y' or redundant PDF artifacts.
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          temperature: 0.1,
          systemInstruction: "You are an institutional data architect. Ensure 100% data retention and structural precision. Do not skip the end of the document."
        }
      });
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'md' | 'pdf' | 'docx') => {
    ...
    try {
      if (type === 'md') {
        ...
        setProcStage('Neural Synthesis: Mapping full standards grid...');
        ...
        <div className="flex items-center justify-between p-4 lg:p-8 border-b dark:border-white/5">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              <h3 className="text-lg lg:text-xl font-black tracking-tight truncate text-slate-900 dark:text-white">Full-Asset Synthesis</h3>
              <p className="text-[10px] lg:text-xs text-slate-500 truncate">{isProcessing ? procStage : 'Deep audit of mapped standards and objectives.'}</p>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col lg:grid lg:grid-cols-2 overflow-hidden">
          <div className="flex flex-col border-b lg:border-b-0 lg:border-r dark:border-white/5 p-4 lg:p-8 bg-slate-50/50 dark:bg-black/20 h-1/2 lg:h-full">
            <label className="text-[10px] font-black uppercase tracking-widest text-indigo-500 mb-3 flex items-center gap-2">
              <ShieldCheck size={12}/> Verified Source (High-Capacity Buffer)
            </label>
            <textarea 
              value={draftMarkdown} 
              ...
              placeholder="Waiting for full-document neural mapping..."
            />
          </div>
          <div className="flex flex-col p-4 lg:p-8 bg-white dark:bg-slate-900 overflow-y-auto custom-scrollbar h-1/2 lg:h-full">
            ...
            <div className="prose dark:prose-invert max-w-none text-xs lg:text-sm" dangerouslySetInnerHTML={{ __html: previewHtml || '<p>Generating deep preview...</p>' }} />
          </div>
        </div>
          ...
          <div className="w-full lg:max-w-md">
            ...
            {!error && draftMarkdown && (
              <p className="text-[10px] lg:text-xs text-emerald-600 font-bold flex items-center gap-2">
                <CheckCircle2 size={14}/> Full Asset Synced (${Math.round(draftMarkdown.length / 1024)} KB). Ready for cloud indexing.
              </p>
            )}
          </div>
          <div className="flex w-full lg:w-auto gap-3">
            <button onClick={goBackToSelection} disabled={isProcessing} className="flex-1 lg:flex-none px-6 py-3 text-slate-400 font-bold hover:text-slate-700 dark:hover:text-slate-200 transition-colors text-sm disabled:opacity-50">
              Abort
            </button>
            <button 
    ...
    <div className="bg-white dark:bg-slate-900 rounded-[2rem] lg:rounded-[3rem] p-6 lg:p-12 w-full max-w-2xl shadow-2xl border border-slate-100 dark:border-white/5 animate-in zoom-in-95 relative overflow-hidden">
      <div className="flex items-center justify-between mb-8 lg:mb-10">
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-indigo-50 dark:bg-indigo-950/30 rounded-full border border-indigo-100 dark:border-indigo-900/20">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-400">High-Capacity Ingestion</span>
        </div>
      </div>
      ...
      <div className="text-center mb-8 lg:mb-12">
        ...
        <p className="text-sm lg:text-base text-slate-500 mt-2 font-medium">Capture 100% of large curriculum standards.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 lg:gap-4">
        <label className="relative group cursor-pointer">
          ...
          <div className="p-4 lg:p-6 border-2 border-dashed border-slate-200 dark:border-white/10 rounded-2xl lg:rounded-3xl hover:border-indigo-500 hover:bg-indigo-50/30 dark:hover:bg-indigo-900/10 transition-all flex items-center gap-4">
            ...
            <div className="text-left flex-1">
              ...
              <p className="text-[10px] text-slate-400">Direct mapping for pre-formatted assets.</p>
            </div>
        ...
        <label className="relative group cursor-pointer">
          ...
          <div className="p-4 lg:p-6 border-2 border-dashed border-slate-200 dark:border-white/10 rounded-2xl lg:rounded-3xl hover:border-amber-500 hover:bg-amber-50/30 dark:hover:bg-amber-900/10 transition-all flex items-center gap-4">
            ...
            <div className="text-left flex-1">
              <h4 className="font-bold text-xs lg:text-sm text-slate-800 dark:text-slate-200">High-Density PDF (Max Capacity)</h4>
              <p className="text-[10px] text-slate-400">Full extraction for complex, multi-page standards.</p>
            </div>
            <ArrowRight size={16} className="text-slate-300 group-hover:translate-x-1 transition-all" />
          </div>
        </label>
        ...
        <label className="relative group cursor-pointer">
          ...
          <div className="p-4 lg:p-6 border-2 border-dashed border-slate-200 dark:border-white/10 rounded-2xl lg:rounded-3xl hover:border-emerald-500 hover:bg-emerald-50/30 dark:hover:bg-emerald-900/10 transition-all flex items-center gap-4">
            ...
            <div className="text-left flex-1">
              ...
              <p className="text-[10px] text-slate-400">Adaptive parsing for raw unformatted curriculum docs.</p>
            </div>
            <ArrowRight size={16} className="text-slate-300 group-hover:translate-x-1 transition-all" />
          </div>
        </label>
      ...
      {isProcessing && (
        <div className="absolute inset-0 bg-white/95 dark:bg-slate-950/95 flex flex-col items-center justify-center rounded-3xl lg:rounded-[3rem] z-50 backdrop-blur-md animate-in fade-in duration-300">
          ...
          <p className="mt-8 px-8 text-center text-[10px] text-slate-400 max-w-sm font-medium">The AI is currently mapping every objective. Large files can take up to 2 minutes.</p>
        </div>
      )}
    </div>
  );
