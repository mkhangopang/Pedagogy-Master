...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const navItems = [
    ...
    { id: 'standards',  label: 'Standards Browser', icon: BookMarked },
    { id: 'tracker',    label: 'Progress Tracker',  icon: ClipboardCheck },
  ];

  // 🏛️ STAKEHOLDER LENS INJECTION
  ...
  const getRoleDisplay = () => {
    if (userProfile.stakeholderRole === StakeholderRole.GOVT_AUDITOR)
      return { icon: <Scale size={18} />, color: 'bg-blue-600', label: 'Govt Auditor' };
    if (userProfile.stakeholderRole === StakeholderRole.NGO_OBSERVER)
      return { icon: <Eye size={18} />, color: 'bg-emerald-600', label: 'NGO Observer' };
  ...
  return (
    <aside className="h-full bg-indigo-950 dark:bg-slate-950 text-white flex flex-col transition-all duration-300 relative w-full border-r border-transparent dark:border-slate-800 shadow-2xl">
      ...
      <div className="p-6 flex items-center justify-between">
        ...
        {onClose && (
          <button onClick={onClose} className="p-2 text-indigo-300 hover:text-white transition-colors lg:hidden">
            <X size={24} />
          </button>
        )}
      </div>
      ...
      <nav className="flex-1 px-4 mt-2 space-y-1 overflow-y-auto custom-scrollbar">
        {navItems.map((item) => {
          ...
          const isStandards = item.id === 'standards';
          return (
            <button
              key={item.id}
            ...
            >
              <Icon
                size={20}
                className={`shrink-0 ${isActive ? 'text-white' : isStandards ? 'text-emerald-400' : 'group-hover:text-white'}`}
              />
              {!isCollapsed && (
                <span className="font-medium truncate text-sm uppercase tracking-tight flex-1 text-left">
                  {item.label}
                </span>
              )}
              {/* NEW badge for Standards Browser */}
              {!isCollapsed && isStandards && !isActive && (
                <span className="text-[8px] font-black uppercase tracking-widest bg-emerald-500 text-white px-1.5 py-0.5 rounded-full shrink-0">
                  NEW
                </span>
              )}
            </button>
      ...
      <div className="p-4 border-t border-white/5 bg-black/10">
        ...
        <button
          onClick={toggleTheme}
          className={`w-full flex items-center gap-3 px-4 py-2.5 mb-2 text-indigo-300/70 hover:text-white rounded-xl transition-colors ${isCollapsed ? 'justify-center' : ''}`}
        >
        ...
        <div className={`flex items-center gap-3 p-3 bg-white/5 rounded-xl mb-2 border border-white/5 ${isCollapsed ? 'justify-center' : ''}`}>
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${roleInfo.color} shadow-sm text-white`}>
            {roleInfo.icon}
          </div>
          {!isCollapsed && (
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-4 py-3 text-rose-400 hover:bg-rose-500/10 rounded-xl transition-colors ${isCollapsed ? 'justify-center' : ''}`}
        >
          <LogOut size={18} />
