    ...
    <div className={`w-full animate-chat-turn mb-6 md:mb-10 ${isAi ? 'bg-transparent' : ''}`}>
      <div className={`flex flex-col gap-3 md:gap-4 mx-auto max-w-full px-4 md:px-8`}>
        ...
        <div className={`flex items-center gap-3 ${isAi ? 'flex-row' : 'flex-row-reverse'}`}>
          ...
          {isAi && content && (
            <button onClick={handleCopy} className="p-1.5 text-slate-400 hover:text-indigo-600 transition-all ml-1 hidden xs:block">
        ...
        <div className={`w-full overflow-hidden ${isAi ? '' : 'flex justify-end'}`}>
          <div className={`relative break-words ${
            isAi 
              ...
              : 'bg-indigo-600 text-white px-4 md:px-5 py-3 rounded-2xl rounded-tr-none shadow-md max-w-[95%] md:max-w-[80%]'
          }`}>
            <div 
              className={`prose dark:prose-invert max-w-full text-sm md:text-[15px] leading-relaxed md:leading-[1.75] ${isAi ? '' : 'text-white'}`}
              dangerouslySetInnerHTML={{ __html: renderedHtml }}
            />

            {isAi && metadata?.sources?.length > 0 && (
              <div className="mt-6 pt-6 border-t border-slate-100 dark:border-white/5 space-y-3">
                <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {metadata.sources.map((source: any, i: number) => (
                    <a 
                      ...
                      className="flex items-center gap-2 px-2.5 py-1 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-lg text-[10px] font-bold text-indigo-600 dark:text-indigo-400 hover:bg-indigo-600 hover:text-white transition-all shadow-sm"
                    >
        ...
        <div className={`flex items-center gap-3 px-1 ${isAi ? 'justify-start' : 'justify-end opacity-40'}`}>
          {isAi && (
            <div className="flex items-center gap-1.5 text-[8px] md:text-[9px] font-black uppercase tracking-widest text-slate-400 bg-slate-100 dark:bg-white/5 px-2 py-0.5 rounded-md">
