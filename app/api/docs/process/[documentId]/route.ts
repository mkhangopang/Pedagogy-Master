...
export async function POST(req: NextRequest, props: { params: Promise<{ documentId: string }> }) {
  ...
  if (!job) {
    ...
    job = { id: jobId, step: IngestionStep.EXTRACT, retry_count: 0 };
  }

  if ((job.retry_count || 0) > 5) {
    const msg = "MAX_RETRIES_EXCEEDED: Ingestion grid failed to stabilize.";
  ...
  await adminSupabase.from('ingestion_jobs').update({ retry_count: (job.retry_count || 0) + 1 }).eq('id', job.id);
  ...
  try {
    ...
    if (job.step === IngestionStep.PARSE) {
      ...
      // Process in chunks of 15k chars to avoid token exhaustion and handle large docs
      const CHUNK_SIZE = 15000;
      const textChunks = [];
      for (let i = 0; i < Math.min(text.length, 60000); i += CHUNK_SIZE) {
      ...
      for (const chunk of textChunks) {
        try {
          ...
          const chunkRecords = JSON.parse(extraction.match(/\[[\s\S]*\]/)?.[0] || '[]');
          records = [...records, ...chunkRecords];
        } catch (e) {
          console.error("LLM chunk extraction failed", e);
        }
      }
      
      // Fallback to regex if LLM failed completely
      if (records.length === 0) {
      ...
      if (records.length > 0) {
        // Deduplicate records by slo_code
        const uniqueRecords = Array.from(new Map(records.map(r => [r.slo_code, r])).values());
        
        await adminSupabase.from('slo_database').delete().eq('document_id', documentId);
        await adminSupabase.from('slo_database').insert(uniqueRecords.map((r: any) => ({
