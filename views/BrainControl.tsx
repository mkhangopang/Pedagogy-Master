...
import { 
  RefreshCw, Zap, Check, ShieldCheck, Terminal, Cpu, Activity, Database, AlertCircle, Server, Globe, BarChart3, Fingerprint, Layers, Rocket
} from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 px-2 text-left">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black flex items-center gap-3 tracking-tight uppercase dark:text-white">
            <Fingerprint className="text-indigo-600" /> Brain v125.4
          </h1>
        </div>
        <div className="flex bg-slate-100 dark:bg-slate-900 p-1 rounded-2xl border dark:border-white/5 overflow-x-auto no-scrollbar">
          {[
            ...
            { id: 'sql', icon: <Terminal size={14}/>, label: 'Protocol' }
          ].map(tab => (
            <button 
              key={tab.id} 
              onClick={() => setActiveTab(tab.id as any)} 
              className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-white dark:bg-slate-800 text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}
            >
              {tab.icon} {tab.label}
            </button>
          ))}
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-200 dark:border-white/5 shadow-2xl space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-black dark:text-white uppercase flex items-center gap-2 tracking-tight">
                <Cpu size={18} className="text-indigo-500" /> Instructional DNA
              </h2>
              <span className="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full text-[9px] font-black uppercase">v{formData.version}.0 Deployment</span>
            </div>
            <textarea 
              ...
              placeholder="Master synthesis logic goes here..."
            />
          ...
          <div className="space-y-6">
             <div className="bg-slate-950 text-white p-8 rounded-[3rem] shadow-2xl relative overflow-hidden flex flex-col gap-8 border border-white/5">
                ...
                <div>
                   ...
                   <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">Real-time Inference Quality</p>
                </div>
                
                <div className="space-y-5 relative z-10">
                   <MetricRow label="RAG PRECISION" value="98.5%" trend="optimal" />
                   <MetricRow label="SYNTAX ALIGN" value="99.2%" trend="optimal" />
                ...
                <div className="pt-6 border-t border-white/10 flex items-center justify-between">
                   <div className="space-y-1">
                      ...
                      <span className="block text-xs font-black text-indigo-400">DMMR v4.0 Active</span>
                   </div>
             ...
             <div className="p-8 bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-100 dark:border-white/5 shadow-sm space-y-6">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400 font-medium leading-relaxed">
                  Surgical Extraction pipeline is optimized for Sindh Board Progression Grids. Automatically linearizes nested tables and generates B-GRADE-DOMAIN codes.
                </p>
           ...
           {gridStatus.map(node => (
             <div key={node.id} className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-xl space-y-6 group hover:border-indigo-500 transition-all">
                ...
                <div>
                   ...
                   <p className="text-[10px] text-slate-400 font-bold uppercase mt-1">{node.id}</p>
                </div>
                {node.status === 'active' ? (
                  ...
                  <div className="p-4 bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/40 rounded-2xl">
                     <p className="text-[9px] font-black text-rose-600 uppercase mb-1">Logic Exception</p>
                     <p className="text-[10px] font-medium text-slate-600 dark:text-slate-400">Node rejected handshake. Exponential backoff active.</p>
                  </div>
                )}
             </div>
           ))}
      ...
      {activeTab === 'dialects' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
           <DialectCard title="Sindh Board (Surgical)" desc="Enforces B-GRADE-DOMAIN-CHAPTER codes. Linearizes Progression Grids automatically." active />
           <DialectCard title="Cambridge IGCSE" desc="Focuses on Assessment Objectives (AO) and specific command words." />
           <DialectCard title="Agha Khan Board" desc="Optimized for conceptual mapping and SLO-aligned exam styles." />
           <DialectCard title="Global Standard" desc="Generic Bloom-based outcomes for independent institutions." />
        </div>
      )}
      ...
      {activeTab === 'sql' && (
        <div className="bg-slate-900 p-10 rounded-[3rem] border border-white/10 shadow-2xl">
           <div className="flex items-center justify-between mb-8">
              <h3 className="text-white font-black uppercase text-xs tracking-widest flex items-center gap-2"><Database size={16} /> Migration Protocol</h3>
              <div className="flex gap-2">
                 <button className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest">Run Migration</button>
              </div>
           </div>
           <pre className="p-6 bg-black/40 rounded-2xl text-emerald-400 font-mono text-[10px] leading-relaxed overflow-x-auto">
{`-- v125: Enhanced SLO Precision Grid
ALTER TABLE public.slo_database ADD COLUMN IF NOT EXISTS chapter_code TEXT;
ALTER TABLE public.slo_database ADD COLUMN IF NOT EXISTS section_code TEXT;
CREATE INDEX IF NOT EXISTS idx_slo_hierarchy ON public.slo_database(chapter_code, section_code);`}
           </pre>
        </div>
      )}
    </div>
...
const MetricRow = ({ label, value, trend }: any) => (
  <div className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5">
    ...
    <div className="text-right">
       ...
       <span className={`block text-[8px] font-black uppercase ${trend === 'optimal' ? 'text-emerald-500' : 'text-rose-400'}`}>{trend}</span>
    </div>
  </div>
);
...
const DialectCard = ({ title, desc, active = false }: any) => (
  <div className={`p-10 rounded-[3rem] border transition-all ${active ? 'bg-indigo-600 border-indigo-400 text-white shadow-2xl scale-105' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-white/5 text-slate-900 dark:text-white shadow-sm'}`}>
     ...
     <div className={`mt-8 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest w-fit ${active ? 'bg-white/10 text-white' : 'bg-slate-100 dark:bg-white/5 text-slate-500'}`}>
        {active ? 'ACTIVE ARCHIVE' : 'AVAILABLE FOR SYNC'}
     </div>
  </div>
);
