...
import { 
  GraduationCap, Loader2, Mail, Lock, ArrowRight, ArrowLeft, 
  CheckCircle2, AlertCircle, Eye, EyeOff, ShieldCheck, KeyRound
} from 'lucide-react';
...
const Login: React.FC<LoginProps> = ({ onBack, onSession, initialMode = 'login' }) => {
      ...
      if (view === 'login') {
        ...
        if (authError) {
           ...
           if (authError.message.includes('Email not confirmed')) {
             throw new Error("Verification Pending: Please check your inbox and verify your email.");
           }
        ...
        if (password !== confirmPassword) throw new Error("Passwords do not match. Please verify your entries.");
        ...
        setView('update-password-success');
      }
    } catch (err: any) {
      setError({ message: err.message || "Authentication service failed to respond." });
      setLoading(false);
    } finally {
      if (view !== 'signup-success' && view !== 'reset-sent' && view !== 'update-password-success') {
        setLoading(false);
      }
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-6">
        <div className="w-full max-w-md bg-white dark:bg-slate-900 p-10 rounded-3xl shadow-xl text-center border border-slate-100 dark:border-white/5">
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Password Reset Link Sent</h2>
          <p className="text-slate-500 mb-6 text-sm">
            We sent a secure password recovery link to <span className="font-semibold text-slate-800 dark:text-slate-200">{email}</span>. Click the link in your email to choose your new password.
          </p>
          <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl mb-6 text-xs text-slate-500 text-left space-y-1.5 border border-slate-100 dark:border-white/5">
            <p className="font-semibold text-slate-700 dark:text-slate-300">Next Steps:</p>
            <p>• Check your inbox and spam folder for an email from Pedagogy Master.</p>
            <p>• The reset link is valid for 60 minutes.</p>
          </div>
          <div className="space-y-3">
            <button onClick={() => setView('login')} className="w-full py-3.5 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-all">
              Return to Sign In
            </button>
            <button onClick={() => setView('forgot-password')} className="w-full py-2.5 text-xs font-semibold text-slate-500 hover:text-indigo-600 transition-colors">
  ...
  if (view === 'update-password-success') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-6">
        <div className="w-full max-w-md bg-white dark:bg-slate-900 p-10 rounded-3xl shadow-xl text-center border border-slate-100 dark:border-white/5">
          <div className="w-20 h-20 bg-emerald-50 dark:bg-emerald-900/20 rounded-full flex items-center justify-center mx-auto mb-6">
          ...
          <p className="text-slate-500 mb-8 text-sm">Your new password has been set securely. You can now access your educator workspace.</p>
          <button 
            onClick={async () => {
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-white/5">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold dark:text-white">
              {view === 'login' ? 'Sign In' : view === 'signup' ? 'Create Account' : view === 'update-password' ? 'Set New Password' : 'Reset Password'}
            </h2>
            {view !== 'login' && view !== 'update-password' && (
              <button onClick={() => setView('login')} className="text-xs font-bold text-slate-400 hover:text-indigo-600 flex items-center gap-1">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            {view !== 'forgot-password' && (
              <div className="space-y-1">
                <div className="flex items-center justify-between ml-1">
                  <label htmlFor="auth-password" className="text-sm font-semibold text-slate-600 dark:text-slate-400">
                    {view === 'update-password' ? 'New Password' : 'Password'}
                  </label>
                  {view === 'login' && (
                    <button type="button" onClick={() => setView('forgot-password')} className="text-[11px] font-bold text-indigo-600 hover:underline">
                      Forgot?
                    </button>
                  )}
                </div>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-12 pr-12 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-white/5 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:outline-none dark:text-white text-sm" 
                    placeholder={view === 'update-password' ? 'Min 8 characters' : '••••••••'} 
                    required 
                  />
                  <button 
                    type="button" 
            ...
            {(view === 'signup' || view === 'update-password') && (
              <div className="space-y-1">
                <label htmlFor="auth-confirm-password" className="text-sm font-semibold text-slate-600 dark:text-slate-400 ml-1">
                  Confirm Password
                </label>
                <div className="relative">
            ...
            <button 
              ...
              className="w-full py-4 bg-indigo-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-600/20 hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50 active:scale-95"
            >
              {loading ? (
                ...
                <>
                  ...
                  {view === 'update-password' && 'Save New Password'}
          ...
          {view === 'login' && (
            <div className="mt-8 pt-6 border-t border-slate-100 dark:border-white/5 text-center">
              <button onClick={() => setView('signup')} className="text-sm font-semibold text-indigo-600 hover:text-indigo-800">
                New educator? Create account
              </button>
            </div>
          )}
          ...
          {view === 'signup' && (
            <div className="mt-8 pt-6 border-t border-slate-100 dark:border-white/5 text-center">
              <button onClick={() => setView('login')} className="text-sm font-semibold text-indigo-600 hover:text-indigo-800">
