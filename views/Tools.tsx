...
const Tools: React.FC<ToolsProps> = ({ brain, documents, onQuery, canQuery, user }) => {
  ...
  // Main Generate Function - Now with real SLO context injection
  const handleGenerate = async (userInput: string, handoffContext?: string) => {
    ...
    try {
      ...
      // === CRITICAL FIX: Real SLO Context Injection ===
      let sloContext = '';
      if (isCurriculumEnabled && activeDoc?.id) {
        const { data: slos } = await supabase
          ...
          .limit(60); // prevent token overflow
...
Instructions:
- Always reference specific SLO codes when relevant (e.g., B09A01).
...
- If the request belongs to another tool, intelligently suggest switching with context transfer.
`;
        ...
        <div className={`fixed inset-y-0 right-0 w-80 bg-white dark:bg-[#0d0d0d] shadow-2xl z-[200] transform transition-transform duration-500 border-l border-slate-100 dark:border-white/5 ${isSliderOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="p-8 flex flex-col h-full">
            <div className="flex items-center justify-between mb-8">
              ...
              <button onClick={() => setIsSliderOpen(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-xl transition-all">
                <X size={20} />
              </button>
            </div>
  ...
  // Artifact + Chat View (keep your original code here)
  return (
    <div className="flex flex-col h-[calc(100vh-120px)] lg:h-[calc(100vh-64px)] bg-slate-50 dark:bg-[#080808] relative overflow-hidden print:h-auto print:overflow-visible">
      {/* Your existing artifact + chat UI code goes here - unchanged */}
      {/* ... (the rest of your original return statement when activeTool is selected) */}
    </div>
  );
};
