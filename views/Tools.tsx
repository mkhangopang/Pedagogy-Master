...
const Tools: React.FC<ToolsProps> = ({ brain, documents, onQuery, canQuery, user }) => {
  ...
  const handleGenerate = async (userInput: string) => {
    ...
    try {
      ...
      // Enhanced prompt based on persona
      ...
      if (effectiveTool === 'visual-aid') {
        const result = await geminiService.generateVisualAid(personaPrompt, effectiveTool, brain, user, activeDoc?.id);
  ...
  const renderArtifact = () => {
    ...
    return (
      <div className="artifact-canvas-container">
        {canvasImage && (
          <div className="mb-8 rounded-[2rem] overflow-hidden border border-slate-200 dark:border-white/10 shadow-xl group relative">
             <img src={canvasImage} alt="Synthesized Visual" className="w-full h-auto object-cover group-hover:scale-[1.01] transition-transform duration-700" />
             <div className="absolute top-4 right-4 flex gap-2">
                <a href={canvasImage} download={`visual_${Date.now()}.png`} className="p-2.5 bg-white/90 dark:bg-black/90 text-slate-900 dark:text-white rounded-xl shadow-xl hover:scale-110 transition-all"><Download size={18}/></a>
             </div>
             <div className="p-4 bg-slate-50 dark:bg-white/5 border-t dark:border-white/5 flex justify-between items-center">
                <span className="text-[10px] font-black uppercase text-indigo-600 tracking-widest flex items-center gap-2"><ImageIcon size={14}/> Integrated Visual Node</span>
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">1024x1024 Precision</span>
             </div>
          </div>
        )}
  ...
  const toolDefinitions = [
    ...
    { id: 'visual-aid', name: 'Visual Hub', icon: ImageIcon, desc: 'Synthetic Educational Aids', color: 'bg-rose-600' },
      ...
      <div className="max-w-5xl mx-auto w-full pt-8 pb-20 px-4 md:px-6 animate-in fade-in duration-500 relative z-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          ...
          <div className="hidden lg:flex items-center gap-4 px-6 py-4 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-800/30 rounded-3xl shadow-sm">
             {/* Fix: Added missing Activity icon import to handle rendering of the status node */}
             <div className="p-2 bg-emerald-500 text-white rounded-xl"><Activity size={18} /></div>
             <div>
                <p className="text-[10px] font-black uppercase text-emerald-600 dark:text-emerald-400 tracking-widest">Neural Status</p>
                <p className="text-xs font-bold text-slate-700 dark:text-slate-300">Operational Grid</p>
             </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {toolDefinitions.map((tool) => (
            <button key={tool.id} onClick={() => setActiveTool(tool.id)} className={`p-6 md:p-8 rounded-[2.5rem] md:rounded-[3rem] border transition-all text-left flex flex-col gap-4 md:gap-6 group bg-white dark:bg-[#111] border-slate-200 dark:border-white/5 hover:border-indigo-500 hover:shadow-2xl`}>
        ...
        <div className="mt-16 p-8 bg-slate-900 rounded-[2.5rem] md:rounded-[3rem] text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl relative overflow-hidden">
           ...
           <div className="flex items-center gap-6 relative z-10">
              <div className="p-4 bg-white/10 rounded-2xl"><Target size={32} className="text-emerald-400" /></div>
              <div className="text-left">
                 <h4 className="text-lg font-black uppercase tracking-tight">Standardized Accuracy</h4>
                 <p className="text-sm text-slate-400 font-medium italic">Verified against institutional RAG clusters.</p>
              </div>
           </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={`flex-1 flex flex-col bg-white dark:bg-[#0a0a0a] transition-all duration-300 ${mobileActiveTab === 'logs' ? 'hidden md:flex' : 'flex'} overflow-hidden`}>
           <div className="px-8 py-4 border-b border-slate-100 dark:border-white/5 flex items-center justify-between shrink-0 bg-white dark:bg-[#0a0a0a] z-10">
              <div className="flex items-center gap-3"><FileEdit size={18} className="text-indigo-600" /><span className="text-[10px] md:text-xs font-black uppercase tracking-widest text-slate-900 dark:text-white">Authoring Canvas</span></div>
              <div className="flex items-center gap-2">
                <button onClick={() => {
                  const cleanText = canvasContent.split('--- Synthesis Node:')[0].trim();
                  navigator.clipboard.writeText(cleanText);
                }} className="px-4 py-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-xl text-slate-600 dark:text-slate-300 transition-all flex items-center gap-2 text-[10px] font-black uppercase tracking-widest border dark:border-white/5"><Copy size={14}/> Copy Content</button>
              </div>
           </div>
