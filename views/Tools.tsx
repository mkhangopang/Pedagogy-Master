...
import { 
  ...
  MessageSquare, FileEdit, ChevronLeft
} from 'lucide-react';
...
const Tools: React.FC<ToolsProps> = ({ brain, documents, onQuery, canQuery, user }) => {
  ...
  const selectedDoc = documents.find(d => d.isSelected) || documents[0];
  ...
  const handleGenerate = async (userInput: string) => {
    ...
    try {
      ...
      // Auto-switch to artifact view on mobile when generation starts
  ...
  if (!activeTool) {
    return (
      <div className="max-w-5xl mx-auto w-full pt-12 pb-20 px-4 md:px-6">
        <div className="flex items-center gap-4 mb-10">
          <div className="p-3 bg-indigo-600 rounded-2xl text-white shadow-xl shadow-indigo-600/20">
            <LayoutPanelLeft size={32} />
          </div>
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tight">Pedagogical Engines</h1>
            <p className="text-slate-500 font-medium text-sm md:text-base">Select a neural node to begin high-fidelity synthesis.</p>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)] lg:h-[calc(100vh-64px)] bg-slate-50 dark:bg-[#080808] relative overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* SIDEBAR: Strategy Logs (Visible in Split/Chat mode or specific Mobile Tab) */}
        <div className={`
        ...
        {/* MAIN: High-Fidelity Canvas (Visible in Split/Canvas mode or specific Mobile Tab) */}
        <div className={`
        ...
        `}>
           ...
           <div className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-12 lg:p-20 bg-slate-50/20 dark:bg-[#0a0a0a]">
              <div className="max-w-4xl mx-auto bg-white dark:bg-[#111] p-8 md:p-12 lg:p-16 rounded-[2rem] shadow-sm border border-slate-100 dark:border-white/5 min-h-full">
