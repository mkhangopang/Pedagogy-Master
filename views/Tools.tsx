...
const Tools: React.FC<ToolsProps> = ({ brain, documents, onQuery, canQuery, user }) => {
  ...
  const handleGenerate = async (userInput: string) => {
    ...
    if (activeTool === 'visual-aid' && isEnterprise) {
      ...
      try {
        ...
        if (data.imageUrl) {
          ...
          setMessages(prev => [...prev, { id: crypto.randomUUID(), role: 'assistant', content: "Neural Visual Aid synthesized successfully. See the Canvas.", timestamp: new Date().toISOString() }]);
          if (window.innerWidth < 768) setMobileActiveTab('artifact');
        } else {
           alert(data.error || "Visual synthesis failed.");
        }
      } catch (e) { console.error(e); } finally { setIsGenerating(false); return; }
    }
    ...
    try {
      ...
      let fullContent = '';
      if (window.innerWidth < 768) setMobileActiveTab('artifact');
      const stream = geminiService.generatePedagogicalToolStream(effectiveTool, userInput, { base64: activeDoc?.base64Data, mimeType: activeDoc?.mimeType, filePath: activeDoc?.filePath }, brain, user, activeDoc?.id);
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-slate-500 font-medium text-sm md:text-lg mt-1 italic flex items-center gap-2">
                {activeDoc ? <><ShieldCheck size={18} className="text-emerald-500" /> Anchored to: {activeDoc.name}</> : <><Info size={18} /> Ready for general assistance.</>}
              </p>
            </div>
          </div>
          <button onClick={() => setIsSliderOpen(true)} className="flex items-center gap-3 px-6 py-3 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl text-xs font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-400 hover:shadow-xl transition-all active:scale-95"><Library size={18} /> Context</button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {toolDefinitions.map((tool) => (
            <button
              key={tool.id}
              onClick={() => {
                if (tool.enterprise && !isEnterprise) { alert("Enterprise node required for Neural Vision features."); return; }
              ...
              className={`p-8 rounded-[3rem] border transition-all text-left flex flex-col gap-6 group shadow-sm hover:shadow-2xl hover:-translate-y-1 ${
                tool.enterprise && !isEnterprise ? 'bg-slate-50 dark:bg-slate-900/50 grayscale border-dashed border-slate-300' : 'bg-white dark:bg-[#111] border-slate-200 dark:border-white/5 hover:border-indigo-500'
              }`}
            >
    ...
    <div className="flex flex-col h-[calc(100vh-120px)] lg:h-[calc(100vh-64px)] bg-slate-50 dark:bg-[#080808] relative overflow-hidden">
      <div className="flex items-center justify-between px-4 md:px-6 py-4 border-b border-slate-100 dark:border-white/5 bg-white dark:bg-[#0a0a0a] z-30">
        <div className="flex items-center gap-3 md:gap-5">
          <button onClick={() => {setActiveTool(null); setMessages([]); setCanvasContent(''); setCanvasImage(null);}} className="p-2.5 bg-slate-50 dark:bg-white/5 text-slate-500 hover:text-slate-900 dark:hover:text-white rounded-2xl transition-all"><ArrowLeft size={20} /></button>
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-[8px] md:text-[9px] font-bold text-emerald-500 uppercase tracking-widest mt-0.5">Neural Mode: {activeTool === 'visual-aid' ? 'Gemini Vision 2.5' : 'Gemini 3 Thinking'}</p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2 md:gap-3">
          <button onClick={() => setIsSliderOpen(true)} className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 rounded-xl text-[10px] font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-400 transition-all hover:bg-slate-100"><Library size={14} /> <span className="hidden xs:inline">Context</span></button>
          <button onClick={() => setMobileActiveTab(mobileActiveTab === 'logs' ? 'artifact' : 'logs')} className={`md:hidden flex items-center gap-2 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg ${mobileActiveTab === 'artifact' ? 'bg-indigo-600 text-white' : 'bg-slate-900 text-white'}`}>{mobileActiveTab === 'artifact' ? <><MessageSquare size={14}/> Logs</> : <><FileText size={14}/> Canvas</>}</button>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className={`flex flex-col border-r border-slate-200 dark:border-white/5 bg-slate-50/50 dark:bg-[#0d0d0d] transition-all duration-500 ${viewMode === 'chat' ? 'w-full' : 'w-full md:w-[380px] lg:w-[450px] shrink-0'} ${viewMode === 'canvas' ? 'hidden' : 'flex'} ${mobileActiveTab === 'artifact' ? 'hidden md:flex' : 'flex'}`}>
          <div className="px-6 py-4 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-white/50 dark:bg-[#0d0d0d]">
            <div className="flex items-center gap-2"><MessageSquare size={14} className="text-slate-400" /><span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Strategy Logs</span></div>
          </div>
          <div ref={scrollRef} className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6 space-y-4">
            {messages.length === 0 ? <div className="flex flex-col items-center justify-center h-full text-center space-y-6 opacity-40"><Bot size={48} className="text-slate-300" /><p className="text-xs md:text-sm font-medium text-slate-400 max-w-[200px]">Prompt the engine to begin synthesis.</p></div> : messages.map((m) => (<MessageItem key={m.id} id={m.id} role={m.role} content={m.content} timestamp={m.timestamp} />))}
          </div>
          <div className="p-4 border-t dark:border-white/5 bg-white dark:bg-[#0d0d0d]"><ChatInput onSend={handleGenerate} isLoading={isGenerating} placeholder={activeTool === 'visual-aid' ? "Describe the diagram or concept..." : "Enhance the synthesis..."} /></div>
        </div>
                ...
                {canvasImage ? (
                  <div className="space-y-6 animate-in fade-in zoom-in-95 duration-700">
                    ...
                    <div className="flex justify-center gap-4">
                      <button onClick={() => { const a = document.createElement('a'); a.href = canvasImage; a.download = 'neural-diagram.png'; a.click(); }} className="px-8 py-3 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl">Download Asset</button>
                    </div>
                  </div>
                ) : canvasContent ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full py-40 text-center space-y-6 opacity-30"><div className="w-20 h-20 bg-slate-50 dark:bg-white/5 rounded-full flex items-center justify-center text-slate-200"><Edit3 size={40} /></div><h2 className="text-xl font-bold text-slate-300">Awaiting Neural Synthesis</h2></div>
                )}
              </div>
           </div>
        </div>
